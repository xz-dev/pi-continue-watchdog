...
1. A Pi with no `PI_PROCESS_DOMAIN_*` declaration creates a private protocol-v2 domain and hosts its single-domain broker in the root Pi process during awaited `session_start`. It records its own PID in `PI_CONTINUE_WATCHDOG_ROOT_PID` and is the sole decision owner while that domain is open.
...
9. Once an inherited Pi loads watchdog and completes `session_start`, broker observation is strict. Root exit makes descendants uncertain/fail-closed; they cannot restart the broker. Zero-gap coverage from `spawn()` until registration requires a cooperating launcher to call `reserveSpawn()` before launch.
10. Protocol-v1 inherited declarations remain connect-only compatibility for already-running legacy brokers; new roots never create or revive that shared broker. Deliberately stripped/replaced declarations and Pi children that do not load watchdog are outside the observable guarantee. No decision-root takeover occurs after root-process exit.
